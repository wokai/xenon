...
neccessary to add the current user to the `dialout` group via
...
Status of USB adapter inferface can be monitored using dmesg `dmesg | grep tty`
...
For prolific chipset, `dmesg | grep pl2303` or `dmesg | grep 1-1` provides
